...
def check_trans_angvel(data, trans_criteria : tp.trans_criteria_t, tmp1, tmp2, tmp3):
    ...
    ang_vel_err = np.array(data.ekf_data.ang_vel - data.ang_vel_des)
    print("ang_vel_err")
    print(ang_vel_err)
    tmp1 = False
    ...
    if (tp.CHECK_MAX_ANG_VEL_ERR_TRANS_CRIT):
        ang_vel_err = np.array(data.ekf_data.ang_vel - data.ang_vel_des)
        ang_vel_err = abs(ang_vel_err)
        if ((ang_vel_err > np.array(trans_criteria.max_ang_vel_err)).any() and not(trans_criteria._or)):
            print("failed 3_2")
            tmp3 = False
            return [tmp1, tmp2, tmp3]
        elif ((ang_vel_err > np.array(trans_criteria.max_ang_vel_err)).any() and (trans_criteria._or)):
            tmp2 = False
        elif ((ang_vel_err < np.array(trans_criteria.max_ang_vel_err)).any() and (trans_criteria._or)):
            if (tmp1):
                pass
...
def transEKFStartTransition(sat, delay : int):
	...
	print("checking for dart start")
	print(mode)
	if( check_transition_criteria(sat.data, tp.EKFStart2DartStart) and 
        (mode == 3)):
		print("beginning dart start")
		return mode_transition(sat,tp.state_t.DARTSTARTPREPARE)
...
def transDartStartTransition(sat, delay : int):
	...
	if( check_transition_criteria( sat.data, tp.ControlStart2Control ) ):
		return mode_transition(sat,tp.state_t.DARTPREPARE)
	...
	sat.adcs_delay = delay # only delay for nominal transition
	return mode_transition(sat,tp.state_t.DARTSTARTCALCULATECONTROL)
...
def transDartTransition(sat, delay : int):
	...
	sat.adcs_delay = delay # only delay for nominal transition
	return mode_transition(sat,tp.state_t.DARTCALCULATECONTROL)
